...
class MosaicApp {
     ...
     */
    buildCanvas(imageSource ) : Promise<HTMLCanvasElement> {
     ...
     */
    loadImage(imageFile : File) : Promise<any> {
        return new Promise((resolve, reject) => {
             if (!imageFile)
            ...
            if (!imageFile.type)
                reject("Image provided was not an image type we know how to handle");
     ...
     */
    buildTileGroups(canvas: HTMLCanvasElement) {
        ...
        imageData.data.forEach((colourData, index) => {
            ...
            let tileGroup = tileGroups[xTile] ? tileGroups[xTile][yTile] : undefined;
            if (!tileGroup) {
                tileGroup = new TileGroup(xTile, yTile);
                if (!tileGroups[xTile]){
                    tileGroups[xTile] = [];
                }
                tileGroups[xTile][yTile] = tileGroup;
            }
     ...
     * @param  {} tileGroups
     */
    renderTileGroups(tileGroups : Object) {
        return new Promise((resolve, reject) => {
            let lastX = 0;
            // let rollingY = 0;
            // let rowOfTiles : Array<TileGroup> = [];
            const rowManager = {};
            const promises = [];
            ...
            Object.values(tileGroups).forEach((rowGroup : Array<TileGroup>) => {
                ...
                // if (lastX > 0 && rowGroup.x == 0) {
                   
                const loadPromises = rowGroup.map(tileGroup => {
